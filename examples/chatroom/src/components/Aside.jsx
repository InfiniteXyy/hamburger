...
import hamburger, { HStack, Image, Input, listen, Text, VStack } from '@hamburger/core';
import { actions, withStore } from '../store';

function FriendItem(friend, currentId) {
  return HStack(
    Image(friend.avatar).size(65).border({ radius: '50%' }).margin({ right: 8 }),
    VStack(
...
function Aside(store) {
  return VStack(
    HStack(Input(store.search).placeholder('Search')).class('search'),
